...
class RPSTest(StageTest):
    ...
    def generate(self) -> List[TestCase]:
        ...
        options = 'rock,paper,scissors,lizard,spock'.split(
            ',')
        ...
        for line in reply.split('\n'):
            ...
            if 'well done' in line.lower():
                for option in win:
                    ...
                    return CheckResult.wrong('Wrong result of the game!' + (
                        "" if not reveal else f"\nOption list used: {options}.\n\"{inp}\" can only beat {win}"))
            elif 'draw' in line.lower() and inp not in line.lower():
                return CheckResult.wrong('Wrong result of the game!' + (
                    "" if not reveal else f"\nOption list used: {options}.\nThe game ends with a draw only when user "
                                          f"and computer choose the same option"))
